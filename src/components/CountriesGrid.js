      ...
      if (
        ...
        axios
          .get(
            filterSelection === "All"
              ? `${base_endpoint}/all`
              : `${base_endpoint}/region/${filterSelection}`
          )
          .then((response) => {
            setIsloading(false);
  ...
  useEffect(() => {
    ...
    if (
      ...
      if (filterSelection === "All") {
        url = `${base_endpoint}/all`;
      } else {
        url = `${base_endpoint}/region/${filterSelection}`;
      }
      setIsloading(true);
      axios
        .get(url)
  ...
  useEffect(() => {
    if (requestCount) {
      ...
      if (
        ...
        url = `${base_endpoint}/name/${props.match.params.countryName.trim()}`;
        ...
        if (
          ...
          url = `${base_endpoint}/name/${inputText.trim()}`;
        }
      }
      axios
        .get(url)
          ...
          if (response.data.status !== 404) {
            ...
            let temp = response.data.filter((prop) => {
              if (
                ...
                return (
                  prop.region.toUpperCase() === filterSelection.toUpperCase()
                );
              }
            });
